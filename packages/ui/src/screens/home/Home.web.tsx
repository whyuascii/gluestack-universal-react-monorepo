    ...
    <div className="min-h-screen bg-gray-100 font-sans text-gray-700 overflow-x-hidden selection:bg-gray-400 selection:text-white">
      ...
      <nav className="relative z-50 px-6 py-6 max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gray-500 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-sm">
            ICON
          </div>
      ...
      <header className="relative z-10 px-6 pt-12 pb-24 md:pt-20 md:pb-32 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-24">
          <div className="flex-1 text-center md:text-left">
            <div className="inline-block px-4 py-2 bg-secondary-100 rounded-full text-gray-700 font-bold mb-6 transform -rotate-2">
              {t("hero.badge")}
            </div>
                ...
                <>
                  <form onSubmit={handleWaitlistSubmit} className="flex flex-col sm:flex-row gap-3">
                    <input
                      ...
                      className="flex-1 px-6 py-4 rounded-full border-2 border-gray-300 bg-white text-gray-700 placeholder:text-gray-700/40 focus:outline-none focus:border-gray-500 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <button
                      type="submit"
                      disabled={isSubmitting}
      ...
      <section id="features" className="relative z-10 py-24 bg-white/50 rounded-t-5xl -mt-12">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            <p className="text-xl text-gray-500 text-2xl">{t("features.subtitle")}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div className="bg-gray-100 p-8 rounded-3xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 group">
              ...
              <p className="text-gray-500 text-sm">{t("features.choreGarden.extra")}</p>
            </div>
            ...
            <div className="bg-gray-100 p-8 rounded-3xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 group">
              ...
              <p className="text-gray-500 text-sm">{t("features.calendar.extra")}</p>
            </div>
            ...
            <div className="bg-gray-100 p-8 rounded-3xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 group">
              ...
              <p className="text-gray-500 text-sm">{t("features.loveNotes.extra")}</p>
            </div>

            {/* Card 4: Stay in Sync (NEW) */}
            <div className="bg-gray-100 p-8 rounded-3xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 border border-gray-200 group">
              <div className="h-48 flex items-center justify-center mb-6 bg-white rounded-3xl group-hover:bg-gray-50 transition-colors">
                <div className="text-gray-400 text-6xl">🔄</div>
              </div>
              <h3 className="text-2xl font-bold text-gray-700 mb-3">
                {t("features.stayInSync.title")}
              </h3>
              <p className="text-gray-600 leading-relaxed mb-3">
                {t("features.stayInSync.description")}
              </p>
              <p className="text-gray-500 text-sm">{t("features.stayInSync.extra")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="about" className="relative z-10 py-24 px-6">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className="text-center mb-20">
            <span className="inline-block px-4 py-1 bg-gray-500/20 rounded-full text-gray-700 font-bold text-xl mb-4">
              {t("navigation.ourStory")}
            </span>
          ...
          {/* Intro & Why we made it */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-24">
            <div className="relative">
              <div className="absolute inset-0 bg-gray-300/20 rounded-3xl transform rotate-3 scale-105"></div>
              <div className="bg-white p-8 md:p-12 rounded-3xl shadow-md relative z-10">
                <h3 className="text-3xl font-bold text-gray-700 mb-6">
                  {t("about.whyWeMadeIt.title")}
                </h3>
                <div className="space-y-6 text-lg text-gray-700/90 leading-relaxed">
                  <p>{t("about.whyWeMadeIt.paragraph1")}</p>
                  <p>{t("about.whyWeMadeIt.paragraph2")}</p>
                  <ul className="space-y-2 ml-6 list-disc marker:text-gray-500">
                    <li>{t("about.whyWeMadeIt.checkInIssues.item1")}</li>
                    <li>{t("about.whyWeMadeIt.checkInIssues.item2")}</li>
                    <li>{t("about.whyWeMadeIt.checkInIssues.item3")}</li>
                  </ul>
                  <p className="mt-6">{t("about.whyWeMadeIt.paragraph3")}</p>
                  <ul className="space-y-3 mt-4">
                    <li className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-lg">
                        ☕
                      </div>
                      <span>{t("about.whyWeMadeIt.tinyDetails.item1")}</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-lg">
                        ✈️
                      </div>
                      <span>{t("about.whyWeMadeIt.tinyDetails.item2")}</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-lg">
                        📝
                      </div>
                      <span>{t("about.whyWeMadeIt.tinyDetails.item3")}</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-lg">
                        🧺
                      </div>
                      <span>{t("about.whyWeMadeIt.tinyDetails.item4")}</span>
                    </li>
                  </ul>
                  <p className="mt-6 text-xl font-semibold text-gray-500">
                    {t("about.whyWeMadeIt.conclusion")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div className="bg-gray-500/10 p-8 rounded-3xl border-2 border-gray-300 border-dashed">
                <p className="text-xl text-gray-700 italic font-medium mb-4">
                  {t("about.intro.paragraph1")}
                </p>
                <div className="flex items-center gap-2 text-gray-500 text-lg">
                  <span className="text-gray-300 text-xl">💛</span>
                  <span>From our nest to yours</span>
                </div>
              </div>

              <div className="bg-white p-8 rounded-3xl shadow-sm">
                <p className="text-lg text-gray-700/90">{t("about.intro.paragraph2")}</p>
              </div>
            </div>
          </div>

          {/* Our Vibe */}
          <div className="max-w-3xl mx-auto text-center bg-white/60 p-10 rounded-3xl border border-gray-200 backdrop-blur-sm">
            <h3 className="text-3xl font-bold text-gray-700 mb-6">{t("about.ourVibe.title")}</h3>
            <p className="text-xl text-gray-700/90 mb-6 leading-relaxed">
              {t("about.ourVibe.paragraph1")} {t("about.ourVibe.paragraph2")}
            </p>
            <p className="text-2xl font-bold text-gray-500">{t("about.ourVibe.conclusion")}</p>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 py-24 px-6 bg-white/50">
        <div className="max-w-5xl mx-auto bg-gray-500 rounded-3xl p-12 md:p-24 text-center relative overflow-hidden shadow-lg">
          ...
          <div className="absolute bottom-0 right-0 w-96 h-96 bg-gray-300/20 rounded-full translate-x-1/3 translate-y-1/3"></div>
              ...
              <div className="max-w-md mx-auto">
                <form onSubmit={handleWaitlistSubmit} className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    required
                    disabled={isSubmitting}
                    className="flex-1 px-6 py-4 rounded-full border-2 border-white/20 bg-white/90 text-gray-700 placeholder:text-gray-700/40 focus:outline-none focus:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                  <button
                    type="submit"
                    disabled={isSubmitting}
      ...
      {/* Footer */}
      <footer className="relative z-10 bg-gray-800 text-white py-16 px-6 rounded-t-3xl">
